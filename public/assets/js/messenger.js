...
const getConversation = function(){
    $.get('/api/convrsations',function(respoonse){
        ...
        for(i in respoonse.data){
            conversation(respoonse.data[i]); 
        }
    });
}

const conversation = function(chat){
    $("#card-list").append(` <a href="${chat.id}"  data-messages="${chat.id}" class="card border-0 text-reset" id="chat-sidebar">
    <div class="card-body">
        <div class="row gx-5">
            <div class="col-auto">
...
$.get(`/api/convrsations/${id}/messages`,function(response){
    $("#chat-name").text(response.conversation.participients[0].name);
    $("input[name=user_id]").val(response.conversation.participients[0].id);
    $("#chat-avatar").attr("src",response.conversation.participients[0].avatar_url);
   for(i in response.messages.data){

    let c = response.messages.data[i].user_id == userId ? "message-out":"";
